...
class CrudNina(object):
    ...
    def __init__(self):
        ...
        self.user = auth.sign_in_with_email_and_password(
            "vidromano@hotmail.com", "senha123")
        self.db = firebase.database()

    def push(self, ra, state):
        data = {}
        data['LocalTime'] = str(datetime.datetime.now().strftime(
    ...
    def getByRa(self,ra):
        json = self.db.child(ra).get(self.user['idToken']).val() #get json
        json['Ra'] = ra #Creating key for identified
        return json
